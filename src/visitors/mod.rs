    ...
    fn visit_compare(&mut self, compare: Compare<'a>) -> Result {
        match compare {
            ...
            Compare::JsonCompare(json_compare) => match json_compare {
                ...
                JsonCompare::ArrayBeginsWith(left, right) => {
                    self.visit_json_array_begins_with(*left, *right, false)
                }
                JsonCompare::ArrayNotBeginsWith(left, right) => {
                    self.visit_json_array_begins_with(*left, *right, true)
                }
                JsonCompare::ArrayEndsInto(left, right) => {
                    self.visit_json_array_ends_into(*left, *right, false)
                }
                JsonCompare::ArrayNotEndsInto(left, right) => {
                    self.visit_json_array_ends_into(*left, *right, true)
                }
