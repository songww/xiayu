...
impl<'a> Mysql<'a> {
    fn visit_regular_equality_comparison(
        &mut self,
        left: Expression<'a>,
        right: Expression<'a>,
    ) -> visitors::Result {
    ...
    fn visit_regular_difference_comparison(
        &mut self,
        left: Expression<'a>,
        right: Expression<'a>,
    ) -> visitors::Result {
...
impl<'a> Visitor<'a> for Mysql<'a> {
    ...
    fn visit_raw_value(&mut self, value: Value<'a>) -> visitors::Result {
        let res = match value {
            ...
            #[cfg(chrono)]
            Value::NaiveTime(time) => time.map(|time| self.write(format!("'{}'", date))),
        };
    ...
    fn visit_less_than(&mut self, left: Expression<'a>, right: Expression<'a>) -> visitors::Result {
        self.visit_numeric_comparison(left, right, "<")?;
...
mod tests {
    ...
    fn test_raw_bytes() {
        let (sql, params) =
            Mysql::build(Select::default().value(Value::bytes(vec![1, 2, 3]).raw())).unwrap();
