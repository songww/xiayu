use super::{
    Column, Comparable, ConditionTree, DefaultValue, ExpressionKind, IndexDefinition, Join,
    JoinData,
};
use crate::{
...
pub enum TableType<'a> {
    ...
    JoinedTable(Box<(Cow<'static, str>, Vec<Join<'a>>)>),
    Query(Box<Select<'a>>),
    Values(Values<'a>),
}
...
impl<'a> Table<'a> {
    ...
    pub fn add_unique_index(mut self, i: impl Into<IndexDefinition<'a>>) -> Self {
        ...
        self.index_definitions
            .push(definition.set_table(self.clone()));
        self
    }
    ...
    pub(crate) fn join_conditions(
        &self,
        inserted_columns: &[Column<'a>],
    ) -> crate::Result<ConditionTree<'a>> {
            ...
            match index {
                ...
                IndexDefinition::Compound(cols) => {
                    ...
                    for right in cols.iter() {
                        let right_cond =
                            join_cond(right)?.unwrap_or(ConditionTree::NegativeCondition);
        ...
        J: Into<JoinData<'a>>,
    {
        match self.typ {
            TableType::Table(table_name) => {
                self.typ = TableType::JoinedTable(Box::new((
                    table_name.to_owned(),
                    vec![Join::Left(join.into())],
                )))
            }
...
impl<'a, E> From<E> for Table<'a>
where
    E: crate::prelude::Entity,
{
    fn from(entity: E) -> Table<'a> {
        Table {
            typ: TableType::Table(entity.tablename().into()),
            alias: None,
            database: None,
